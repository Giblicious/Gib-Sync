...
const compatible:ClientCompatibility={clientVersion:"0.8.48",clientProtocol:7,minimumVersion:"0.8.48",recommendedVersion:"0.8.48",serverProtocol:7,serverVersion:"0.8.48",serverCapabilities:["readable-generation-v1","external-delete-proof-v1","folder-manifest-v1"],compatible:true,updateAvailable:false,reason:null};
...
describe("server compatibility gate",()=>{
  it("blocks an old server even when it claims the client is compatible",()=>{
    ...
    expect(enforceServerCompatibility({...compatible,serverVersion:"0.8.48-beta.1"})).toMatchObject({compatible:false});
  });
  it("blocks matching versions that omit required safety capabilities",()=>{
    expect(staleServerReason({protocolVersion:7,serverVersion:"0.8.48",serverCapabilities:["readable-generation-v1","external-delete-proof-v1"]})).toContain("folder-manifest-v1");
  });
