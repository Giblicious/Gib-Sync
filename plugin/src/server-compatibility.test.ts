...
const compatible:ClientCompatibility={clientVersion:"0.8.34",clientProtocol:6,minimumVersion:"0.8.34",recommendedVersion:"0.8.34",serverProtocol:6,serverVersion:"0.8.34",serverCapabilities:["readable-generation-v1","external-delete-proof-v1"],compatible:true,updateAvailable:false,reason:null};
...
describe("server compatibility gate",()=>{
  it("blocks an old server even when it claims the client is compatible",()=>{
    ...
    expect(enforceServerCompatibility({...compatible,serverVersion:"0.8.34-beta.1"})).toMatchObject({compatible:false});
  });
  it("blocks matching versions that omit required safety capabilities",()=>{
    expect(staleServerReason({protocolVersion:6,serverVersion:"0.8.34",serverCapabilities:["readable-generation-v1"]})).toContain("external-delete-proof-v1");
  });
