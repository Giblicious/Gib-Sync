...
export class GibSyncSettingTab extends PluginSettingTab {
  ...
  display(){this.unsubscribe?.();if(this.liveRenderTimer!==null)window.clearTimeout(this.liveRenderTimer);this.liveRenderTimer=null;this.containerEl.empty();this.containerEl.addClass("gib-sync-settings");this.containerEl.createEl("h2",{text:"Gib Sync"});const configured=Boolean(this.plugin.settings.deviceToken);
    ...
    if(Platform.isMobile)new Setting(this.containerEl).setName("Maximum mobile file size").setDesc("Files larger than this many MB remain safely stored on the server, Seafile, and capable devices instead of exhausting mobile memory. Set 0 only if this phone can reliably write very large files.").addText((text)=>text.setValue(String(this.plugin.settings.mobileMaxFileSizeMb)).onChange(async(value)=>{const parsed=Number(value);if(!Number.isFinite(parsed))return;this.plugin.settings.mobileMaxFileSizeMb=Math.max(0,Math.min(1024,Math.round(parsed)));this.plugin.requireFullScan();await this.plugin.saveSettings();}));
  ...
  private renderLive(){if(!this.liveRoot)return;const root=this.liveRoot;const previousLog=root.querySelector<HTMLElement>(".gib-sync-activity-log"),previousScroll=previousLog?.scrollTop??0,wasAtTop=!previousLog||previousScroll<4,detailsOpen=root.querySelector<HTMLDetailsElement>(".gib-sync-status-details")?.open??false,activityOpen=root.querySelector<HTMLDetailsElement>(".gib-sync-activity-details")?.open??true;root.empty();const live=this.plugin.liveStatus;const server=this.plugin.serverStatus;const storage=server?.storage||this.plugin.settings.storage,state=this.plugin.indicatorState();
    ...
    const activity=root.createEl("details",{cls:"gib-sync-activity gib-sync-activity-details"});activity.open=activityOpen;const title=activity.createEl("summary",{cls:"gib-sync-activity-title"});title.createEl("strong",{text:"Recent activity"});const buttons=title.createDiv();
    buttons.onclick=(event)=>event.stopPropagation();
    ...
    const log=activity.createDiv({cls:"gib-sync-activity-log"});for(const entry of [...live.activities].reverse().slice(0,20)){const row=log.createDiv({cls:`gib-sync-activity-row is-${entry.level}`});row.createEl("time",{text:new Date(entry.at).toLocaleTimeString()});row.createSpan({cls:"gib-sync-activity-marker"});row.createEl("span",{text:`${entry.message}${(entry.repeats??1)>1?` · repeated ${entry.repeats} times`:""}`});}if(!live.activities.length)log.createEl("div",{cls:"gib-sync-muted",text:"No activity yet. New sync events will appear here."});log.scrollTop=wasAtTop?0:previousScroll;
  }
}
