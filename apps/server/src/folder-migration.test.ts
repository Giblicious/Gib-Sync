...
describe("legacy folder migration repair",()=>{
  ...
  it("plans an explicit retirement directive after a direct unsafe seed was rewound",()=>{
    const {store,vaultId,parentId,headId}=fixture();expect(repairUnsafeLegacyFolderHeads(store)).toBe(1);
    expect(planRetiredLegacyFolderRepair(store,vaultId,parentId)).toMatchObject({originIds:[headId],contaminatedFolders:["A","B","C","D"],currentFolders:[],desiredFolders:[]});store.db.close();
  });
