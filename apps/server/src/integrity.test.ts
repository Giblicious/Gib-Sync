...
describe("head integrity audit",()=>{
  ...
  it("fails closed when a repair directive does not point to a verified unsafe seed",()=>{const {store,vaultId,id,hash,snapshot}=fixture();store.run("INSERT INTO blobs(vault_id,hash,size,created_at) VALUES(?,?,?,?)",vaultId,hash,1,new Date().toISOString());snapshot.folderRepair={retiredFolders:["Stale"],observedAt:new Date().toISOString(),originSnapshotIds:[randomUUID()]};store.run("UPDATE snapshots SET manifest_json=? WHERE id=?",JSON.stringify(snapshot),id);expect(auditHeadIntegrity(store,vaultId,id)).toMatchObject({valid:false,issues:["folder repair provenance is missing or invalid"]});store.db.close();});
});
