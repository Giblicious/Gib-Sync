...
describe("client compatibility",()=>{
  ...
  it("blocks missing, old, and protocol-incompatible clients",()=>{
    const base={minimumVersion:"0.8.19",recommendedVersion:"0.8.20",serverProtocol:7,serverVersion:"0.8.48",serverCapabilities:["readable-generation-v1"]};
  ...
  it("warns without blocking a compatible client behind the recommendation",()=>{
    expect(clientCompatibility({clientVersion:"0.8.19",clientProtocol:7,minimumVersion:"0.8.18",recommendedVersion:"0.8.20",serverProtocol:7,serverVersion:"0.8.48",serverCapabilities:["readable-generation-v1"]})).toMatchObject({compatible:true,updateAvailable:true,reason:null});
  });
});
