...
export function assessChanges(previous:ManifestEntry[],next:ManifestEntry[],policy:SafeguardPolicy,signals?:ClientSafetySignals):{assessment:ChangeAssessment;changes:ChangeItem[]}{
  ...
  const deletionPercent=deleted.length/baseline*100;
  const staleDeletionCount=Math.max(3,Math.ceil(policy.deletionCount/2));
  const staleDeletionPercent=Math.max(10,policy.smallVaultDeletionPercent/2);
  if(signals?.staleBaseline&&deleted.length>=3&&(deleted.length>=staleDeletionCount||deletionPercent>=staleDeletionPercent))
    reasons.push(`An out-of-date device would delete ${deleted.length} files`);
