...
describe("atomic snapshot committer",()=>{
  ...
  it("persists the immutable folder repair issue time",async()=>{const root=mkdtempSync(join(tmpdir(),"gib-sync-commit-repair-"));roots.push(root);const store=new Store(root),vaultId=randomUUID(),now=new Date().toISOString();store.run("INSERT INTO vaults(id,name,wrapped_key,created_at) VALUES(?,?,?,?)",vaultId,"Vault","wrapped",now);const storage={put:async()=>{}},committer=new SnapshotCommitter(store,storage as never,()=>({} as never));const snapshot=await committer.accept({vaultId,parentId:null,deviceId:"server",deviceName:"Server",message:"Repair",entries:[],folders:[],folderRepair:{retiredFolders:["Stale"],observedAt:now,originSnapshotIds:[randomUUID()],issuedAt:now}});expect(snapshot?.folderRepair?.issuedAt).toBe(now);expect(store.getSnapshot(snapshot!.id)?.folderRepair?.issuedAt).toBe(now);store.db.close();});
});
